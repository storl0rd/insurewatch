...
async def submit_claim(claim: ClaimSubmission, request: Request):
    start = time.time()
    # LAB 2: Manual span REMOVED — no span wraps this handler
    # TODO: add tracer.start_as_current_span("submit_claim") context manager
    # TODO: set span attributes for claim.customer_id, claim.type, claim.amount, claim.status
    apply_chaos()
